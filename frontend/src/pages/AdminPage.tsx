...
import { createEmployee, deactivateUser, fetchEmployees, updateUser } from "@/store/usersSlice";
...
export function AdminPage() {
  ...
  const onAdd = async (data: { name: string; email: string; password?: string; department?: string }) => {
    ...
    if (createEmployee.fulfilled.match(result)) {
      toast.success(`Employee created. Password: ${result.payload.temporary_password}`);
      setOpen(false);
      reset();
    ...
    <section className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="overflow-x-auto">
          {loading ? <Skeleton className="h-32 w-full" /> : employees.length === 0 ? (
            <p className="text-muted-foreground">No employees yet.</p>
          ) : (
            <table className="w-full text-sm min-w-[500px]">
              <thead><tr className="border-b"><th className="p-2">Name</th><th className="p-2">Email</th><th className="p-2">Department</th><th className="p-2">Actions</th></tr></thead>
              <tbody>
                {employees.map((e) => (
                  <tr key={e.id} className="border-b">
                    ...
                    <td className="p-2 flex gap-2">
                      <Button size="sm" variant="outline" onClick={() => { setEditId(e.id); reset({ name: e.name, email: e.email, department: e.department || "" }); }}>Edit</Button>
                      <Button size="sm" variant="destructive" onClick={async () => { if (confirm("Deactivate?")) { const r = await dispatch(deactivateUser(e.id)); if (deactivateUser.fulfilled.match(r)) toast.success("Deactivated"); else toast.error("Failed to deactivate"); } }}>Deactivate</Button>
                    </td>
                  </tr>
                ))}
              </tbody>
